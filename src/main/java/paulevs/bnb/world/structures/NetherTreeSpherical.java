...
public class NetherTreeSpherical extends Structure {
	...
	public NetherTreeSpherical(BlockState log, BlockState leaves, BlockState fur, BlockState lantern, float radiusScale) {
	...
	private void makeLeaves(Level level, int x, int y, int z, float radius, Random random) {
		...
		double r2Small = (radius - 1) * (radius - 1);
			...
			for (int j = (int) -radius; j <= radius; j++) {
				...
				if (d < r2) {
					for (int k = (int) -radius; k <= radius; k++) {
						float k2 = k * k;
						if (d + k2 - random.nextInt(2) < r2) {
							k2 = k + 2;
							k2 *= k2;
							if (d + k2 + random.nextInt(2) > r2Small) {
								int py = y + k;
