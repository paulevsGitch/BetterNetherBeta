package paulevs.bnb.world.structure.common;

import net.minecraft.block.Block;
import net.minecraft.level.Level;
import net.minecraft.level.structure.Structure;
import net.modificationstation.stationapi.api.block.BlockState;
import net.modificationstation.stationapi.api.util.math.MathHelper;

import java.util.Random;

public class BiomeBlendingStructure extends Structure {
	private final BlockState state;
	private final int randomRange;
	private final int randomMin;
	private final int radiusSqr;
	private final int radius;
	private final int height;
	
	public BiomeBlendingStructure(Block block, int radius, int height) {
		this(block.getDefaultState(), radius, height);
	}
	
	public BiomeBlendingStructure(BlockState state, int radius, int height) {
		this.state = state;
		this.radius = radius;
		this.height = height;
		this.randomMin = radius >> 1;
		this.randomRange = randomMin << 1 | 1;
		this.radiusSqr = radius * radius;
	}
	
	@Override
	public boolean generate(Level level, Random random, int x, int y, int z) {
		random.setSeed(MathHelper.hashCode(x, y, z));
		for (int dx = -radius; dx <= radius; dx++) {
			int wx = x + dx;
			for (int dy = -height; dy <= height; dy++) {
				int wy = y + dy;
				for (int dz = -radius; dz <= radius; dz++) {
					int wz = z + dz;
					int xsq = dx + random.nextInt(randomRange) - randomMin;
					int zsq = dz + random.nextInt(randomRange) - randomMin;
					if (xsq * xsq + zsq * zsq > radiusSqr) continue;
					if (!level.getBlockState(wx, wy, wz).isOf(Block.NETHERRACK)) continue;
					if (!level.getBlockState(wx, wy + 1, wz).getMaterial().isReplaceable()) continue;
					level.setBlockState(wx, wy, wz, state);
				}
			}
		}
		return true;
	}
}
