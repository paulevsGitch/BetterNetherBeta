...
public class ObsidianBoatItem extends TemplateBoatItem {
	...
	public ItemStack use(ItemStack stack, Level level, PlayerEntity player) {
		...
		float cosY = MCMath.cos(-player.yaw * toDeg - pi);
		float sinY = MCMath.sin(-player.yaw * toDeg - pi);
		float cosP = -MCMath.cos(-player.pitch * toDeg);
		Vec3D end = start.add(sinY * cosP * 5.0F, MCMath.sin(-player.pitch * toDeg) * 5.0F, cosY * cosP * 5.0F);
